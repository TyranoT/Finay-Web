...
function coerce(payload: unknown): Lugar[] {
  if (Array.isArray(payload)) return payload as Lugar[];
  if (payload && typeof payload === "object") {
    const obj = payload as Record<string, unknown>;
    for (const chave of ["lugares", "items", "data"]) {
      if (Array.isArray(obj[chave])) return obj[chave] as Lugar[];
    }
  }
  return [];
}
